...
\title{Transitional Representation: \\A Formalism for Conflicting and Evolving Data}

\author{Lars R\"onnb\"ack}

\date{\footnotesize Initial rework --- 4 March 2026}
...
% Bibliography is embedded directly in this file to avoid BibTeX/Biber dependencies.
\usepackage{url}
...
\noindent transitional, representation, posit, assertion, bitemporality, concurrency, reliability, uncertainty
\end{abstract}
...
A \emph{posit} is a triple, \(\left[D, v, t_{\mathrm{app}}\right]\), where the first element is an appearance set, the second a data value, and the third a time point. The domain \(\mathbb{t}\) from which time points are taken is called \emph{appearance time}.}{posit}

\noindent
\textbf{Interpretation.} A posit states that an appearance set \(D\) has value \(v\) from (and including) appearance time \(t_{\mathrm{app}}\) onward, until another posit with the same appearance set becomes applicable.
...
\noindent
We write this compactly as \(\assert(s_{id}, p_{id}, c, T_{\mathrm{ast}})\).
...
\assert(D, p_{2},\;&& 1.0,\;&& \textrm{Friday})
\end{alignat*}

\section{Epistemic Properties of the Model}
\subsection{A signed certainty scale}
\noindent
Humans often communicate confidence linguistically rather than numerically. Table~\ref{Table:certainties2} suggests a coarse mapping.
...
\begin{figure}[htb]
\centering
\includegraphics[width=0.7\linewidth]{v_and_not_v.pdf}
...
A body of information is said to be \emph{symmetric} iff the assertions \(\assert(S, p, {-c}, T)\) and \(\assert(S, \bar{p}, c, T)\) are equivalent for a posit \(p\) and its complement \(\bar{p}\).}{symmetry}
...
A body of information is said to be \emph{bounded} iff the certainties in the assertions \(\assert(S, p, c, T)\) and \(\assert(S, \bar{p}, d, T)\) satisfy \(\abs{c + d} = \abs{c + d}^2\), i.e.\ \(c + d \in \{-1, 0, 1\}\).}{boundary}
...
\noindent
From here on all bodies of information are assumed to be comprehensive. Two further properties pertain to the \emph{information in effect} rather than to individual assertions:
...
$$\frac{1}{2}\sum_{i=1}^{n}\left[1-\frac{c_i}{|c_i|}\right]+\sum_{i=1}^{n}c_i\leq 1.$$}{noncontradiction}

\noindent
\textsc{Example.} Suppose Donna has two assertions simultaneously in effect at \((\textrm{Sunday}, \textrm{10:03})\):
\[a_5 = \assert(D,\,p_4,\;0.25,\;\textrm{Sunday})\quad\text{and}\quad a_6 = \assert(D,\,p_2,\;{-0.75},\;\textrm{Sunday})\]
(``possibly fluffy red'' and ``probably not shaved clean''). Computing the left-hand side:
\[\frac{1}{2}\!\left[1-\frac{0.25}{0.25}+1-\frac{-0.75}{0.75}\right]+0.25-0.75= \tfrac{1}{2}[0+2]-0.5 = 0.5\leq 1.\]
Donna is well within bounds and is not contradicting herself. Note that a positor may simultaneously assert \(c=-1\) (``certainly not'') for an unlimited number of posits without ever violating the inequality.
...
a_1 = \assert(D, p_{2},\;&& 1.0,\;&& \textrm{Friday}) \\
a_2 = \assert(C, p_{1},\;&& 0.75,\;&& \textrm{Friday}) \\
a_3 = \assert(C, p_{3},\;&& -1.0,\;&& \textrm{Friday})
\end{alignat*}
...
a_4 = \assert(D, p_{2},\;&& 0.0,\;&& \textrm{Sunday}) \\
...
For two assertions, \(\assert(S, p, c, T)\) and \(\assert(S, p', c, T)\), with \(p=[D,v,t_{\mathrm{app}}]\) and \(p'=[D,v,t'_{\mathrm{app}}]\) such that \(t_{\mathrm{app}} < t'_{\mathrm{app}}\), the latter is a \emph{restatement} of the former.}{restatement}
...
\noindent
OWL-Time~\cite{OWLTime} provides a rich ontology of temporal instants, intervals, and Allen's calculus of ordering relations; it is well suited to representing when a state holds in the world (\emph{appearance time}, or valid time in database terminology). The W3C specification explicitly notes, however, that the ``valid time'' requirement is not resolved directly: only a user-defined specialisation of \texttt{time:hasTime} is suggested for that purpose~\cite{OWLTime}. More critically, OWL-Time has no concept of \emph{when a source's belief about a state was recorded or revised}. Assertion time---the second temporal axis of transitional representation---has no representation in OWL-Time or in any standard RDF vocabulary.
...
\noindent
Transitional representation treats concurrency, uncertainty, and bitemporality as first-class aspects of a single representation construct: posits and assertions. The \emph{information in effect} procedure enables query-time conflict handling without destructive updates.
...
\noindent
Future work (deferred from the earlier draft) includes schema/model bootstrapping within the representation itself, identification under uncertainty, and constraint evaluation (e.g., higher-order cardinality) over indecisive bodies of information.
...
\bibitem{Ayer}
Alfred Ayer.
\newblock Language, Truth, and Logic.
\newblock \emph{London: V. Gollancz ltd}, 1936.
...
\newblock \emph{Appearance and Reality: A Metaphysical Essay}.
\newblock Routledge, 2016.

\bibitem{Chisholm}
Roderick Chisholm and others.
\newblock \emph{Person and Object: A Metaphysical Study}.
...
\bibitem{CummingEtAl}
Graeme S. Cumming and John Collier.
\newblock Change and identity in complex systems.
\newblock \emph{Ecology and Society}, 10(1), 2005.
...
\bibitem{Hartmann}
Sven Hartmann.
\newblock On the implication problem for cardinality constraints and functional dependencies.
\newblock \emph{Annals of Mathematics and Artificial Intelligence}, 33(2--4):253--307, 2001.
...
\bibitem{Inmon}
William H. Inmon.
\newblock \emph{Building the Data Warehouse}.
...
\bibitem{Koubarakis}
Manolis Koubarakis.
\newblock Representation and querying in temporal databases: the power of temporal constraints.
\newblock In \emph{Data Engineering, 1993. Proceedings. Ninth International Conference on}, pages 327--334.
  IEEE, 1993.
...
\newblock Relational change and relational changes.
\newblock \emph{Philosophical Studies}, 34(1):63--79, 1978.
...
\newblock Schema evolution in heterogeneous database architectures, a schema transformation approach.
\newblock In \emph{International Conference on Advanced Information Systems Engineering}, pages 484--499.
  Springer, 2002.
...
\newblock Ontology evolution: Not the same as schema evolution.
\newblock \emph{Knowledge and Information Systems}, 6(4):428--440, 2004.
...
\newblock \emph{Studies in Subjective Probability}, pages 25--52, 1926.
...
\bibitem{Sider}
Theodore Sider.
\newblock Quantifiers and temporal ontology.
\newblock \emph{Mind}, 115(457):75--97, 2006.
...
\bibitem{Thalheim}
Bernhard Thalheim.
\newblock The art of conceptual modelling.
\newblock \emph{Information Modelling and Knowledge Bases XXII. Frontiers in Artificial Intelligence and Applications}, 237:149--168, 2012.
