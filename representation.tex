...
\begin{abstract}
Modern information systems increasingly need to represent information that is concurrent (multiple sources and perspectives), bitemporal (what was believed when, about what happened when), and uncertain (graded confidence and explicit disagreement). Existing representation formalisms typically handle at most one or two of these dimensions without either losing provenance or forcing early conflict resolution. This paper proposes \emph{transitional representation}, a formalism whose atomic unit is the \emph{posit}: an \(n\)-ary, time-stamped statement. Assertions are represented as ordinary posits over reserved roles, carrying signed certainty and source identity without introducing a second primitive construct. We also formalize a minimal class layer showing how model-level distinctions can be represented inside the same formalism. Finally, we give deterministic temporal-resolution procedures that can be applied uniformly to different posit layers; the familiar global \emph{information in effect} then appears as the special case induced by assertion posits.
\end{abstract}
...
\noindent
\textsc{Contributions.} This rework focuses on: (i) posits as the single atomic representation construct, with assertions treated as ordinary posits over reserved roles, (ii) a compact class layer showing how model-level distinctions are represented inside the same formalism, (iii) a signed certainty scale \(c\in[-1,1]\), and (iv) deterministic temporal-resolution procedures that support both ordinary posit cuts and the global \emph{information in effect} slice for assertion bodies, as well as clause-local temporal cuts chosen at query time. Richer typing and full constraint enforcement are deferred to future work. Identification remains outside the representation layer by design.

\noindent
\textsc{Scope and evaluation stance.} The present article is a formal and conceptual contribution. Its purpose is to define the representation, clarify its semantics, and position it relative to adjacent data-management formalisms. It does \emph{not} claim a complete theory of schema evolution, constraint reasoning, or source-fusion policy, nor does it present a comparative systems benchmark. The implementation notes included later serve only to show feasibility and to connect the formalism to an executable system.
...
[\{(p_{1}, \textit{posit}), (C, \textit{ascertains})\},\;&& 0.75,\;&& \textrm{Friday}] \\
[\{(p_{2}, \textit{posit}), (D, \textit{ascertains})\},\;&& 1.0,\;&& \textrm{Friday}]
\end{alignat*}
...
\begin{alignat*}{3}
q_{1} = [\{(C_{\mathrm{person}}, \textit{named})\},\;&& \textit{Person},\;&& \textrm{Friday}] \\
q_{2} = [\{(A, \textit{thing}), (C_{\mathrm{person}}, \textit{class})\},\;&& \textit{active},\;&& \textrm{1972\text{-}08\text{-}20}] \\
q_{3} = [\{(C_{\mathrm{suspect}}, \textit{named})\},\;&& \textit{Suspect},\;&& \textrm{Friday}] \\
q_{4} = [\{(A, \textit{thing}), (C_{\mathrm{suspect}}, \textit{class})\},\;&& \textit{active},\;&& \textrm{Friday}] \\
[\{(q_{1}, \textit{posit}), (M, \textit{ascertains})\},\;&& 1.0,\;&& \textrm{Friday}] \\
[\{(q_{2}, \textit{posit}), (M, \textit{ascertains})\},\;&& 1.0,\;&& \textrm{Friday}] \\
[\{(q_{3}, \textit{posit}), (I, \textit{ascertains})\},\;&& 1.0,\;&& \textrm{Friday}] \\
[\{(q_{4}, \textit{posit}), (I, \textit{ascertains})\},\;&& 0.75,\;&& \textrm{Friday}]
\end{alignat*}

\noindent
Here \(q_{1}\) and \(q_{3}\) name two classes, while \(q_{2}\) says that Archie (\(A\)) belongs to the class \textit{Person} from his birth onward and \(q_{4}\) says that Archie belongs to the class \textit{Suspect} from Friday onward. The two claims live at different levels of abstraction, may be asserted by different positors, and are queryable and revisable like any other posits. The same mechanism also supports conflicting classifications. One positor may assert that Archie is a \textit{Suspect} with certainty \(1\), while another asserts the same classifier with certainty \(0\), or instead asserts a competing classifier such as \textit{Witness} or \textit{Victim}. Such disagreement is represented in exactly the same way as disagreement about beard state or event participation.

\noindent
A query over the information in effect can therefore treat class memberships and domain facts uniformly: ``Who was classified as a suspect on Friday?'' is handled by the same retrieval machinery as ``Who was observed with a beard at 10:00?'' The role of the class layer in the present paper is not to provide a full schema theory, but to show that model-level distinctions can enter the same bitemporal and source-sensitive representation as first-order assertions. Subclass relations, class transitions, and richer modeling constructs can be expressed in the same style, but are left outside the present minimal formalization.
...
A body of information is said to be \emph{bounded} iff the certainties in the assertion posits \([\{(p,\textit{posit}),(S,\textit{ascertains})\},c,T]\) and \([\{(\bar{p},\textit{posit}),(S,\textit{ascertains})\},d,T]\) satisfy \(\abs{c + d} = \abs{c + d}^2\), i.e.\ \(c + d \in \{-1, 0, 1\}\).}{boundary}
...
\noindent
Without universality, disagreement cannot be interpreted as disagreement about the same referent. Together, the four properties yield:
...
\[a_5 = [\{(p_4,\textit{posit}),(D,\textit{ascertains})\},0.25,\textrm{Sunday}]\quad\text{and}\quad a_6 = [\{(p_2,\textit{posit}),(D,\textit{ascertains})\},-0.75,\textrm{Sunday}]\]
...
a_1 = [\{(p_{2}, \textit{posit}), (D, \textit{ascertains})\},\;&& 1.0,\;&& \textrm{Friday}] \\
a_2 = [\{(p_{1}, \textit{posit}), (C, \textit{ascertains})\},\;&& 0.75,\;&& \textrm{Friday}] \\
a_3 = [\{(p_{3}, \textit{posit}), (C, \textit{ascertains})\},\;&& -1.0,\;&& \textrm{Friday}]
\end{alignat*}
...
a_4 = [\{(p_{2}, \textit{posit}), (D, \textit{ascertains})\},\;&& 0.0,\;&& \textrm{Sunday}] \\
a_5 = [\{(p_{4}, \textit{posit}), (D, \textit{ascertains})\},\;&& 0.25,\;&& \textrm{Sunday}]
\end{alignat*}
...
p_{4} = [\{(A, \textit{has beard})\},\;&& \textrm{fluffy red},\;&& \textrm{10:02}] \\
a_2 = [\{(p_{1}, \textit{posit}), (C, \textit{ascertains})\},\;&& 0.75,\;&& \textrm{Friday}] \\
a_7 = [\{(p_{4}, \textit{posit}), (C, \textit{ascertains})\},\;&& 0.75,\;&& \textrm{Friday}] \\
a_8 = [\{(p_{1}, \textit{posit}), (C, \textit{ascertains})\},\;&& 0.75,\;&& \textrm{Sunday}] \\
a_9 = [\{(p_{4}, \textit{posit}), (C, \textit{ascertains})\},\;&& 0.25,\;&& \textrm{Sunday}]
\end{alignat*}
...
\noindent
A reference implementation named Positorium~\cite{Positorium} demonstrates the feasibility of the approach. Positorium is an experimental database engine written in Rust~\cite{Matsakis} that stores data as posits and exposes a query language, Traqula, for interacting with them. The implementation is cited here only as evidence that the formalism can be executed in a concrete system; details of system design, optimization, and query ergonomics lie outside the scope of the present paper. Interested readers can consult the public repository for the current state of the implementation.
